...
# loading in forestry plot chrons
fp.chrons <- read.csv("../data_gathering_cleaniing/processed_data/morton_forestry_two_thirds_chrons.csv", header=T)

fp.chron.short <- fp.chrons[fp.chrons$year >=1980 & fp.chrons$year < 2020,]
...
# response variables will be the plot chrons
vars.resp <- tolower(names(fp.chron.short)[!names(fp.chron.short) %in% "year"])
...
write.csv(mod.out, file.path("processed_data", "tr_analyses","Morton_forestry_plot_TR_ClimateCorrs_Daily.csv"), row.names=F)  
# ----------------------------------

# ----------------------------------
# Plotting and exploring the output
# ----------------------------------
mod.out <- read.csv(file.path("processed_data", "tr_analyses","Morton_forestry_plot_TR_ClimateCorrs_Daily.csv"))
summary(mod.out)
...
mod.out$resp <- as.factor(mod.out$resp)

PRGn5 <- c("#7b3294", "#c2a5cf", "gray50", "#a6dba0", "#008837")

pdf("figures/critical_periods_morton_df/forestry_plot_ClimateCorr_0.01_t-stat_.pdf", height=8, width=11)
...
pdf(file="figures/critical_periods_morton_df/forestry_plot_ClimateCorr_5day__t-stat.pdf", height=8, width=11)
...
pdf(file = "figures/critical_periods_morton_df/forestry_plot_ClimateCorr_5day_r-val.pdf", height=8, width=10)
for(i in unique(mod.out$pred)){
print(
