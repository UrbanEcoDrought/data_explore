...
#########################
#Using ndvi.anomaly as response variable
...
ChicagolandSPINDVI <- merge (ChicagolandSPI, ndvi.test, by=c("date"), all.x=F, all.y=TRUE)
ChicagolandSPINDVI <- na.omit(ChicagolandSPINDVI)
...
days.use <- unique(ChicagolandSPINDVI$doy[ChicagolandSPINDVI$month >=3 & ChicagolandSPINDVI$month <=9])
days.use 
resp.vars <- c("ndvi.anomaly")
pred.vars <- c("X14d.SPI", "X30d.SPI", "X60d.SPI", "X90d.SPI")

RESP <- resp.vars[1] #(ndvi.anomaly)
...
tstat_NDVI.anomaly_all.SPI_Urban.Medium <- ggplot(data=mod.out[mod.out$p.val<0.05,]) +
  facet_grid(.~RESP) +
  geom_tile(aes(x=DOY, y=PRED, fill=t.stat)) +
  scale_x_continuous(breaks=month.breaks$doy, labels=month.breaks$month)+
  labs(title = "t.stat of Response (NDVI.anomaly) and Predictors (all SPI) for Urban Medium when p-value is significant")

png(file="G:/Shared drives/Urban Ecological Drought/data/r_files/figures/LME tstats/tstat_NDVI.anomaly_all.SPI_Urban.Medium.png", unit="in", height = 5, width = 10, res = 300)
plot(tstat_NDVI.anomaly_all.SPI_Urban.Medium)
dev.off()

############################
#Adding Response nested loop
...
tstat_NDVI.obs_and_anomaly_all.SPI_Urban.Medium <- ggplot(data=mod.out[mod.out$p.val<0.05,]) +
  facet_grid(.~RESP) +
  geom_tile(aes(x=DOY, y=PRED, fill=t.stat)) +
  scale_x_continuous(breaks=month.breaks$doy, labels=month.breaks$month)+
  labs(title = "t.stat of Responses (NDVI.obs and anomaly) and Predictors (all SPI) for Urban Medium when p-value is significant")

png(file="G:/Shared drives/Urban Ecological Drought/data/r_files/figures/LME tstats/tstat_NDVI.obs_and_anomaly_all.SPI_Urban.Medium.png", unit="in", height = 5, width = 10, res = 300)
plot(tstat_NDVI.obs_and_anomaly_all.SPI_Urban.Medium)
dev.off()


##########################
#Running nested for loop with ndvi obs and anomaly as RESP and all SPI and vpd as PRED
...
colnames(ChicagolandSPINDVIVPDNA)[15] = 'VPD'
...
tstat_NDVI.obs_all.SPI.VPD_Urban.Medium <- ggplot(data=mod.out[mod.out$p.val<0.05,]) +
  facet_grid(.~RESP) +
  geom_tile(aes(x=DOY, y=PRED, fill=t.stat)) +
  scale_x_continuous(breaks=month.breaks$doy, labels=month.breaks$month)+
  labs(title = "t.stat of Response (NDVI.obs) and Predictors (all SPI and VPD) for Urban Medium when p-value is significant")

png(file="G:/Shared drives/Urban Ecological Drought/data/r_files/figures/LME tstats/tstat_NDVI.obs_all.SPI.VPD_Urban.Medium.png", unit="in", height = 5, width = 10, res = 300)
plot(tstat_NDVI.obs_all.SPI.VPD_Urban.Medium)
dev.off()

################################
##########################
#Adding VPD as predictor variable
...
tstat_NDVI.obs_and_anomaly_all.SPI.VPD_Urban.Medium <- ggplot(data=mod.out[mod.out$p.val<0.05,]) +
  facet_grid(.~RESP) +
  geom_tile(aes(x=DOY, y=PRED, fill=t.stat)) +
  scale_x_continuous(breaks=month.breaks$doy, labels=month.breaks$month)+
  labs(title = "t.stat of Response (NDVI.obs/anomaly) and Predictors (all SPI and VPD) for Urban Medium when p-value is significant")

png(file="G:/Shared drives/Urban Ecological Drought/data/r_files/figures/LME tstats/tstat_NDVI.obs_and_anomaly_all.SPI.VPD_Urban.Medium.png", unit="in", height = 5, width = 10, res = 300)
plot(tstat_NDVI.obs_and_anomaly_all.SPI.VPD_Urban.Medium)
dev.off()

AIC_NDVI.obs_and_anomaly_all.SPI.VPD_Urban.Medium <- ggplot(data=mod.out[mod.out$p.val<0.05,]) +
  facet_grid(.~RESP) +
  geom_tile(aes(x=DOY, y=PRED, fill=AIC)) +
  scale_x_continuous(breaks=month.breaks$doy, labels=month.breaks$month)+
  labs(title = "AIC of Response (NDVI.obs/anomaly) and Predictors (all SPI and VPD) for Urban Medium when p-value is significant")

png(file="G:/Shared drives/Urban Ecological Drought/data/r_files/figures/LME tstats/AIC_NDVI.obs_and_anomaly_all.SPI.VPD_Urban.Medium.png", unit="in", height = 5, width = 10, res = 300)
plot(AIC_NDVI.obs_and_anomaly_all.SPI.VPD_Urban.Medium)
dev.off()
