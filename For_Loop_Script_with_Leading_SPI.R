...
days.use 
lc.type <- unique(ChicagolandSPINDVI$type)
...
for(RESP in resp.vars){
  for(PRED in pred.vars){
    ...
    dat.tmp <- ChicagolandSPINDVI[ChicagolandSPINDVI$doy>=dayNOW-7 & ChicagolandSPINDVI$doy<=dayNOW+7 & ChicagolandSPINDVI$type==TYPE,]
...
t.stat_NDVI.obs.anomaly_all.SPI_with.leads_all.lc.types <- ggplot(data=mod.out[mod.out$p.val<0.0005,]) +
  ...
  labs(title = "t.stat of Response (NDVI.obs/anomaly) and Predictors (all SPI and leads) for all LC Types when p-value is < 0.005")

png(file="G:/Shared drives/Urban Ecological Drought/data/r_files/figures/LME tstats/t.stat_NDVI.obs.anomaly_all.SPI_with.leads_all.lc.types.png", unit="in", height = 5, width = 10, res = 300)
plot(t.stat_NDVI.obs.anomaly_all.SPI_with.leads_all.lc.types)
dev.off()
