...
ndvi.2023 <- ndvi.latest[ndvi.latest$year==2023,] #subset data to only contain the current year
dim(ndvi.2023)
...
nmonths <- length(unique(lubridate::month(ndvi.2023.grass$date))) # Number of knots per month
nObs <- nrow(ndvi.2023.grass) # If we want to pick the number of knots based our the number of obs
...
# Create a prediction matrix for "time 0"
...
# Create a prediciton matrix for a slight offset so we can calutate the difference for our approximation (time 1)
newD <- newDF
