...
#creating 5 day minimum NDVI for each doy by lc type

lc.type <- unique(ChicagolandTempSPEISPINDVIVPDNA$type) 
doyrange <- 10:301
ChicagolandTempSPEISPINDVIVPDNA$VCI.5day<-NA
ChicagolandTempSPEISPINDVIVPDNA$VCI5.14d.Lag<-NA
#ChicagolandTempSPEISPINDVIVPDNA$VCI3.30d.Lag<-NA
#ChicagolandTempSPEISPINDVIVPDNA$VCI3.45d.Lag<-NA

for(t in 1:length(lc.type)) {
  for(d in 2:(length(doyrange)-2)){
    Fivedaywindow <- subset(ChicagolandTempSPEISPINDVIVPDNA,
                             type==lc.type[t]&doy>(doyrange[d]-3)&doy<(doyrange[d]+3))
    ndvi.5daymin<-min(Fivedaywindow$ndvi.obs)
    ndvi.5daymax<-max(Fivedaywindow$ndvi.obs)
    years<-unique(Fivedaywindow$year[which(Fivedaywindow$doy==doyrange[d])])
    
    for(y in 1:length(years)){
      
      obs<-Fivedaywindow$ndvi.obs[which(Fivedaywindow$doy==doyrange[d]&Fivedaywindow$year==years[y])]
      VCI5day<-((obs - ndvi.5daymin)/(ndvi.5daymax - ndvi.5daymin))
      ...
      ChicagolandTempSPEISPINDVIVPDNA$VCI.5day[index]<-VCI5day
      ...
      for(D in 100:300){
        ...
        if(length(x)>0){
          ...
          ChicagolandTempSPEISPINDVIVPDNA$VCI5.14d.Lag[index2] <-ChicagolandTempSPEISPINDVIVPDNA$VCI.5day[x]
...
summary(ChicagolandTempSPEISPINDVIVPDNA)
saveRDS(ChicagolandTempSPEISPINDVIVPDNA, file.path(google.drive, "data/data_sets/Chicagoland5dVCI.RDS"))
...
Chicagoland5dVCIGrowSeason <- subset(ChicagolandTempSPEISPINDVIVPDNA[ChicagolandTempSPEISPINDVIVPDNA$doy>=91 & ChicagolandTempSPEISPINDVIVPDNA$doy<=304,])

summary(Chicagoland5dVCIGrowSeason)

gam5dVCI.basic <- gam(VCI.5day ~ s(year) + type, data=Chicagoland5dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.basic)
...
gam5dVCI.tmin30.spei30.tmax60.VCIlag.VPD.interact <-gam(VCI.5day ~ s(year) + s(SPEI.X30d, TMAX60d) + s(TMIN30d, doy, by=type) + s(VCI5.14d.Lag) + s(VPD) + type, data=Chicagoland5dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.tmin30.spei30.tmax60.VCIlag.VPD.interact)
