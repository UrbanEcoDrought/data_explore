...
#5dVCI data saved on Google Drive
#ChicagolandTempSPEISPINDVIVPDNA <- readRDS(file.path(google.drive, "data/data_sets/Chicagoland5dVCI.RDS"))
...
#creating 7 day minimum NDVI for each doy by lc type

lc.type <- unique(ChicagolandTempSPEISPINDVIVPDNA$type) 
doyrange <- 10:301
ChicagolandTempSPEISPINDVIVPDNA$VCI.7day<-NA
ChicagolandTempSPEISPINDVIVPDNA$VCI7.14d.Lag<-NA
#ChicagolandTempSPEISPINDVIVPDNA$VCI3.30d.Lag<-NA
#ChicagolandTempSPEISPINDVIVPDNA$VCI3.45d.Lag<-NA

for(t in 1:length(lc.type)) {
  for(d in 2:(length(doyrange)-3)){
    Sevendaywindow <- subset(ChicagolandTempSPEISPINDVIVPDNA,
                            type==lc.type[t]&doy>(doyrange[d]-4)&doy<(doyrange[d]+4))
    ndvi.7daymin<-min(Sevendaywindow$ndvi.obs)
    ndvi.7daymax<-max(Sevendaywindow$ndvi.obs)
    years<-unique(Sevendaywindow$year[which(Sevendaywindow$doy==doyrange[d])])
    
    for(y in 1:length(years)){
      
      obs<-Sevendaywindow$ndvi.obs[which(Sevendaywindow$doy==doyrange[d]&Sevendaywindow$year==years[y])]
      VCI7day<-((obs - ndvi.7daymin)/(ndvi.7daymax - ndvi.7daymin))
      ...
      ChicagolandTempSPEISPINDVIVPDNA$VCI.7day[index]<-VCI7day
      ...
      for(D in 100:300){
        ...
        if(length(x)>0){
          ...
          ChicagolandTempSPEISPINDVIVPDNA$VCI7.14d.Lag[index2] <-ChicagolandTempSPEISPINDVIVPDNA$VCI.7day[x]
        ...
        #        if(length(xy)>0){
        #          
        #          ChicagolandTempSPEISPINDVIVPDNA$VCI5.30d.Lag[index2] <-ChicagolandTempSPEISPINDVIVPDNA$VCI.5day[xy]
        #        }
        ...
        #        if(length(xyz)>0){
        #          
        #          ChicagolandTempSPEISPINDVIVPDNA$VCI5.45d.Lag[index2] <-ChicagolandTempSPEISPINDVIVPDNA$VCI.5day[xyz]
        #        }
      }
    }
...
summary(ChicagolandTempSPEISPINDVIVPDNA)
saveRDS(ChicagolandTempSPEISPINDVIVPDNA, file.path(google.drive, "data/data_sets/Chicagoland7dVCI.RDS"))
...
Chicagoland7dVCIGrowSeason <- subset(ChicagolandTempSPEISPINDVIVPDNA[ChicagolandTempSPEISPINDVIVPDNA$doy>=91 & ChicagolandTempSPEISPINDVIVPDNA$doy<=298,])

summary(Chicagoland7dVCIGrowSeason)

gam7dVCI.basic <- gam(VCI.5day ~ s(year) + type, data=Chicagoland7dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam7dVCI.basic)

gam5dVCI.tmin30.spei30 <-gam(VCI.5day ~ s(year) + s(SPEI.X30d) + s(TMIN30d) + type, data=Chicagoland5dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.tmin30.spei30)

gam5dVCI.tmin30.spei30.tmax60 <-gam(VCI.5day ~ s(year) + s(SPEI.X30d, TMAX60d) + s(TMIN30d) + type, data=Chicagoland5dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.tmin30.spei30.tmax60)

gam5dVCI.double.penalty <- gam(VCI.5day ~ s(year) + s(X14d.SPI) + s(X30d.SPI) + s(X60d.SPI) + s(X90d.SPI) + s(SPEI.X14d) + s(SPEI.X30d) + s(SPEI.X60d) + s(SPEI.X90d) + s(TMIN14d) + s(TMIN30d)+ s(TMIN60d)+ s(TMIN90d) + s(TMAX14d) + s(TMAX30d)+ s(TMAX60d)+ s(TMAX90d) + s(VPD) + type, data=Chicagoland5dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.double.penalty)

gam5dVCI.double.vars <- gam(VCI.5day ~ s(year) + s(X14d.SPI) + s(TMIN90d) + s(TMAX90d) + s(VPD) + type, data=Chicagoland5dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.double.vars)

gam5dVCI.double.vars <- gam(VCI.5day ~ s(year) + s(X14d.SPI) + s(TMIN90d) + s(TMAX90d) + s(VPD) + type, data=Chicagoland5dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.double.vars)

gam5dVCI.tmin30.spei30.tmax60.VCIlag <-gam(VCI.3day ~ s(year) + s(SPEI.X30d, TMAX60d) + s(TMIN30d) + s(VCI3.14d.Lag) + type, data=Chicagoland3dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.tmin30.spei30.tmax60.VCIlag)

gam7dVCI.tmin30.spei30.tmax60.VCIlag.VPD.interact <-gam(VCI.7day ~ s(year) + s(SPEI.X30d, TMAX60d) + s(TMIN30d, doy, by=type) + s(VCI7.14d.Lag) + s(VPD) + type, data=Chicagoland7dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam7dVCI.tmin30.spei30.tmax60.VCIlag.VPD.interact)

gam5dVCI.tmin30.spei30.tmax60.VCIlag.VPD.interact2 <-gam(VCI.3day ~ s(year) + s(SPEI.X30d) + s(TMIN30d, doy, by=type) + s(TMAX60d) + s(VCI3.14d.Lag) + s(VPD) + type, data=Chicagoland3dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.tmin30.spei30.tmax60.VCIlag.VPD.interact2)

gam5dVCI.tmin30.spei30.tmax60.VCIlag.VPD.interact3 <-gam(VCI.3day ~ s(year) + s(SPEI.X30d, TMAX60d, doy, by=type) + s(TMIN30d, doy, by=type) + s(VCI3.14d.Lag) + s(VPD, doy, by=type) + type, data=Chicagoland3dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.tmin30.spei30.tmax60.VCIlag.VPD.interact3)

gam5dVCI.tmin30.spei30.tmax60.VCIlag30d <-gam(VCI.3day ~ s(year) + s(SPEI.X30d, TMAX60d) + s(TMIN30d) + s(VCI3.45d.Lag) + type, data=Chicagoland3dVCIGrowSeason, select=TRUE, method = 'REML')
summary(gam5dVCI.tmin30.spei30.tmax60.VCIlag30d)

#Creating diagnostic plots for best fit VCI model
Chicagoland5dVCIGrowSeason$predicted.5dVCI.interact<-NA
Chicagoland5dVCIGrowSeason$resids.5dVCI.interact <-NA
Chicagoland5dVCIGrowSeason<-na.omit(Chicagoland3dVCIGrowSeason)

Chicagoland5dVCIGrowSeason$predicted.5dVCI.interact <- predict(gam5dVCI.tmin30.spei30.tmax60.VCIlag.VPD.interact)
Chicagoland5dVCIGrowSeason$resids.5dVCI.interact <- resid(gam5dVCI.tmin30.spei30.tmax60.VCIlag.VPD.interact)
...
modeled.obs.3dVCI.interact <- plot(pred.factor.5dVCI ~ factor.5dVCI, data=Chicagoland5dVCIGrowSeason); abline(a=0, b=1, col="red")
