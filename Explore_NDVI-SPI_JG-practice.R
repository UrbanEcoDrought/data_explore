...
tstat_NDVI.obs_X14d.SPI_Urban.Medium <- ggplot(data=mod.out[mod.out$p.val<0.05,]) +
  facet_grid(.~RESP) +
  geom_tile(aes(x=DOY, y=PRED, fill=t.stat)) +
  scale_x_continuous(breaks=month.breaks$doy, labels=month.breaks$month)
plot(tstat_NDVI.obs_X14d.SPI_Urban.Medium)

#Save graph of the t.stat values from lme model of urban.medium with X14d.SPI (Predictor) and NDVI.obs (Response) over grow season with p-value > 0.05 as png.
png(file="G:/Shared drives/Urban Ecological Drought/data/r_files/figures/LME tstats/tstat_NDVI.obs_X14d.SPI_Urban.Medium.png", unit="in", height = 5, width = 10, res = 300)
plot(tstat_NDVI.obs_X14d.SPI_Urban.Medium)
dev.off()
...
# THIS IS THE MAIN GRAPH THAT WE WANT TO SAVE --> JILLIAN: PLEASE SAVE THESE AS PNG FILES IN YOUR WORKFLOW 
# (The others are shown just to show how we built to this)
tstat_NDVI.obs_X30d.SPI_Urban.Medium <- ggplot(data=mod.out[mod.out$p.val<0.05,]) +
  facet_grid(.~RESP) +
  geom_tile(aes(x=DOY, y=PRED, fill=t.stat)) +
  scale_x_continuous(breaks=month.breaks$doy, labels=month.breaks$month)
plot(tstat_NDVI.obs_X30d.SPI_Urban.Medium)

#Save graph of the t.stat values from lme model of urban.medium with X30d.SPI (Predictor) and NDVI.obs (Response) over grow season with p-value > 0.05 as png.
png(file="G:/Shared drives/Urban Ecological Drought/data/r_files/figures/LME tstats/tstat_NDVI.obs_X30d.SPI_Urban.Medium.png", unit="in", height = 5, width = 10, res = 300)
plot(tstat_NDVI.obs_X30d.SPI_Urban.Medium)
dev.off()
...
# THIS IS THE MAIN GRAPH THAT WE WANT TO SAVE --> JILLIAN: PLEASE SAVE THESE AS PNG FILES IN YOUR WORKFLOW 
# (The others are shown just to show how we built to this)
tstat_NDVI.obs_X60d.SPI_Urban.Medium <- ggplot(data=mod.out[mod.out$p.val<0.05,]) +
  facet_grid(.~RESP) +
  geom_tile(aes(x=DOY, y=PRED, fill=t.stat)) +
  scale_x_continuous(breaks=month.breaks$doy, labels=month.breaks$month)
plot(tstat_NDVI.obs_X60d.SPI_Urban.Medium)

#Save graph of the t.stat values from lme model of urban.medium with X60d.SPI (Predictor) and NDVI.obs (Response) over grow season with p-value > 0.05 as png.
png(file="G:/Shared drives/Urban Ecological Drought/data/r_files/figures/LME tstats/tstat_NDVI.obs_X60d.SPI_Urban.Medium.png", unit="in", height = 5, width = 10, res = 300)
plot(tstat_NDVI.obs_X60d.SPI_Urban.Medium)
dev.off()
...
# THIS IS THE MAIN GRAPH THAT WE WANT TO SAVE --> JILLIAN: PLEASE SAVE THESE AS PNG FILES IN YOUR WORKFLOW 
# (The others are shown just to show how we built to this)
tstat_NDVI.obs_X90d.SPI_Urban.Medium <- ggplot(data=mod.out[mod.out$p.val<0.05,]) +
  facet_grid(.~RESP) +
  geom_tile(aes(x=DOY, y=PRED, fill=t.stat)) +
  scale_x_continuous(breaks=month.breaks$doy, labels=month.breaks$month)
plot(tstat_NDVI.obs_X90d.SPI_Urban.Medium)

#Save graph of the t.stat values from lme model of urban.medium with X90d.SPI (Predictor) and NDVI.obs (Response) over grow season with p-value > 0.05 as png.
png(file="G:/Shared drives/Urban Ecological Drought/data/r_files/figures/LME tstats/tstat_NDVI.obs_X90d.SPI_Urban.Medium.png", unit="in", height = 5, width = 10, res = 300)
plot(tstat_NDVI.obs_X90d.SPI_Urban.Medium)
dev.off()
...
###################################
## Iterating the model through the day of year and pred, but keeping the resp the same
###################################
# specifying our subsets for days/responses/predictors
## Each level of a loop will iterate through ONE of these at a time --> one loop will go through days; that would be nested in a model that goes through predictors and that would be nested in one that goes through responses
## But the base model just does ONE LEVEL OF EACH AT A TIME
PRED <- pred.vars[1:4] # x14d.SPI, x30d.SPI, x60d.SPI, X90d.SPI 
RESP <- resp.vars[1] # ndvi.obs; NOTE: For some reason the anomaly wasn't working!  Very weird.
...
for(i in 1:length(days.use)){
  ...
  for(i in 1:length(pred.vars)){
    dat.tmp$PRED[1:4] <-dat.tmp[pred.vars]
