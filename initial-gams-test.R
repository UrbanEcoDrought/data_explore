...
gam_2024 <- gam(NDVI ~ s(yday), data=ndvi.2024) #create simple gam using the day of the year and NDVI
...
newDF <- data.frame(yday=seq(1,254)) #create new data frame with column to represent day of year sequence
...
ggplot(ndata,aes(x=yday,y=fit)) + geom_line() + geom_ribbon(data=ndata, aes(ymin=right_lwr, ymax=right_upr),alpha=0.1) #plot fitted values with 95% CI
